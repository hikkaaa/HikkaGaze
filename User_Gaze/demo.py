...
def parse_args():
    ...
    parser.add_argument(
        '--gpu', dest='gpu_id', help='GPU device id to use [None]',
...
if __name__ == '__main__':
    ...
    batch_size = 1
    cam = args.cam_id
    device = torch.device('cpu')  # Use CPU as the device
    ...
    # Move list to appropriate device and convert to torch.Tensor
    idx_tensor = torch.FloatTensor([idx for idx in range(90)]).to(detector.device)
    x=0
